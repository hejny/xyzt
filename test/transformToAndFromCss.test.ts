...
describe('Conversion of Transform object to CSS', () => {
    ...
    it('is working with z deg CSS rotate ', () => {
        expect(transformToStyleCss(transformFromStyle(`rotate(60deg)`))).toBe(
            `rotate(60deg)`,
        );
    });

    it('is working with z grad CSS rotate ', () => {
        expect(transformToStyleCss(transformFromStyle(`rotate(100grad)`))).toBe(
            `rotate(90deg)`,
        );
    });
    ...
    it('is working with z rad CSS rotate ', () => {
        expect(
            transformToStyleCss(
                transformFromStyle(`rotate(${Math.PI / 3}rad)`),
            ),
        ).toBe(`rotate(60deg)`);
    });
    ...
    it('is working with z turn CSS rotate ', () => {
        expect(
            transformToStyleCss(transformFromStyle(`rotate(-0.75turn)`)),
        ).toBe(`rotate(90deg)`);
    });
