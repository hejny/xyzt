...
describe('Extracting values from CSS/SVG style', () => {
    it('extracts correct values', () => {
        expect(
            extractValuesFromStyle(`translate`, `translate(1px,2px,3px)`)?.map(
                ({ value }) => value,
            ),
        ).toEqual([1, 2, 3]);
        expect(
    ...
    it('can deal with incorrect input', () => {
        expect(
            extractValuesFromStyle(`translate`, `foo(-1.5px -2.1px -0.5px)`),
        ).toEqual(null);
    ...
    it('extracts correct units', () => {
        ...
        expect(
            extractValuesFromStyle(
                `translate`,
                `translate(-1.5% -2.1% -0.5vh)`,
            )?.map(({ unit }) => unit),
        ).toEqual(['%', '%', 'vh']);
        expect(
            extractValuesFromStyle(`rotate`, `rotate(25grad)`)?.map(
                ({ unit }) => unit,
            ),
        ).toEqual(['grad']);
    });
